...
// mracsys
// Remove static type from hId in order
// to use it as the global heap for the hack
s32 hId = -1;
...
// mracsys
// remove dependency on processor.h
...
{
	// mracsys
	// VC iosCreateHeap requires a pointer to the new heap.
	// Homeboy creates a 0xD000 heap, larger than the USB heap requested here, should be safe.
	// Moved heap creation to the init hook, guaranteed to run before this.
...
static s32 USBV5_GetDescriptors(s32 device_id, usb_devdesc *udd)
{
	...
	if (retval==IPC_OK) {
		...
		// mracsys
		// swapped calloc for malloc/memset
		udd->configurations = iosAlloc(hId, udd->bNumConfigurations * sizeof(*udd->configurations));
		...
		for (iConf = 0; iConf < udd->bNumConfigurations; iConf++)
		{
			...
			// mracsys
			// swapped calloc for malloc/memset
			ucd->interfaces = iosAlloc(hId, sizeof(*ucd->interfaces));
			if (ucd->interfaces == NULL) goto free_bufs;
			memset(ucd->interfaces, 0, sizeof(*ucd->interfaces));
			...
			if(uid->extra_size>0)
			{
				uid->extra = iosAlloc(hId, uid->extra_size);
				if(uid->extra == NULL)
					goto free_bufs;
			...
			if (uid->bNumEndpoints) {
				// mracsys
				// swapped calloc for malloc/memset
				uid->endpoints = iosAlloc(hId, uid->bNumEndpoints * sizeof(*uid->endpoints));
...
s32 USB_GetDescriptors(s32 fd, usb_devdesc *udd)
{
	...
	for(iConf = 0; iConf < udd->bNumConfigurations; iConf++)
	{
		...
		// mracsys
		// swapped calloc for malloc/memset
		ucd->interfaces = iosAlloc(hId, ucd->bNumInterfaces * sizeof(*ucd->interfaces));
		...
		for(iInterface = 0; iInterface < ucd->bNumInterfaces; iInterface++)
		{
			...
			// mracsys
			// Suppress sign-compare warning as the for loop guarantees
			// ucd->bNumInterfaces > 1
			if (iInterface==(u32)(ucd->bNumInterfaces-1) && size>2) {
				...
				if (size-retval >= USB_DT_INTERFACE_SIZE && ptr[retval+1] == USB_DT_INTERFACE) {
					...
					// mracsys
					// replace realloc to remove stdlib dependency
					usb_interfacedesc *interfaces;
					if (ucd->interfaces == NULL)
					{
						// should always start with one interface defined
						goto free_and_error;
					}
					else
					{
						interfaces = iosAlloc(hId, (iInterface+2)*sizeof(*interfaces));
						if (interfaces == NULL)
							goto free_and_error;
						memcpy(interfaces, ucd->interfaces, (iInterface+1)*sizeof(*interfaces));
						iosFree(hId, ucd->interfaces);
					}
	...
	if(udd->configurations != NULL)
	{
		for(iConf = 0; iConf < udd->bNumConfigurations; iConf++)
		{
			...
			if(ucd->interfaces != NULL)
			{
				...
				{
					uid = ucd->interfaces+iInterface;
					iosFree(hId, uid->endpoints);
					iosFree(hId, uid->extra);
				}
				iosFree(hId, ucd->interfaces);
			}
		}
		iosFree(hId, udd->configurations);
	}
}
