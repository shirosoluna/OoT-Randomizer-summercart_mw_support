...
|`0x019F`|`CFG_BOSSES`|6|`0xBD`|For non mixed pools, the bosses are listed twice in two different order. First for each dungeon entrance in the order of Deku, DC, Jabu, Forest, Fire, Water, Shadow, Spirit, BotW, Ice, GTG and Ganons castle, a null-terminated `0x8`-byte (`0x9` including the null terminator) ASCII string containing the boss located in it, padded with spaces on the right. And then for each dungeon in the order of Deku, DC, Jabu, Forest, Fire, Water, Shadow, Spirit and Ganons, a null-terminated `0x8`-byte (`0x9` including the null terminator) ASCII string containing the boss located in it, padded with spaces on the right. For mixed pools, only one list is needed, and the string displays the hint area the boss is located in instead.|

# Version 7

This version is currently exclusive to the `dev-fenhl` and `riir` branches and is subject to change. Checking the [branch identifier](https://wiki.ootrandomizer.com/index.php?title=Branch_identifiers) and randomizer version number is recommended.
...
|`0x004A`|`TRIFORCE_PIECES_REQUIRED`|4|`0x02`|In Triforce hunt, the total number of Triforce pieces (across all worlds) required to win the game.|
|`0x004C`|`SPECIAL_DEAL_COUNTS`|4|`0x08`|A byte representing the number of special deal slots in each shop, in the following order: KF Shop, Market Bazaar, Market Potion Shop, Market Bombchu Shop, Kak Bazaar, Kak Potion Shop, GC Shop, ZD Shop.|
|`0x0054`|`CFG_DUNGEON_REWARD_AREAS`|7|`0x09`|For each dungeon reward in the order of Emerald, Ruby, Sapphire, Light, Forest, Fire, Water, Shadow, and Spirit, the hint area where that reward can be found, as a value of the `opt_hint_area_t` enum defined in `dungeon_info.h`.|
|`0x0123`|`CFG_ADULT_TRADE_SHUFFLE`|5|`0x01`|`1` if the item-cycling functionality for the adult trade inventory slot is enabled.|
|`0x0124`|`CFG_CHILD_TRADE_SHUFFLE`|5|`0x01`|`1` if the item-cycling functionality for the child trade inventory slot is enabled.|
|`0x0125`|`CFG_DUNGEON_BOSS_INFO`|6|`0x0E`|The first two bytes indicates what shuffles are enabled for dungeons and bosses (0 : none, 1 : in their own pools, 2 : mixed pools). The following 12 bytes indicates the map index of the dungeons located respectively at Deku, DC, Jabu, Forest, Fire, Water, Shadow, Spirit, BotW, Ice, GTG and Ganons castle. Since GTG and Ganons Castle have no map, their index is put at a fake value (64)|
|`0x0133`|`CFG_DUNGEON_ENTRANCES`|6|`0x6C`|For non mixed pools : for each dungeon entrance in the order of Deku, DC, Jabu, Forest, Fire, Water, Shadow, Spirit, BotW, Ice, GTG and Ganons castle, a null-terminated `0x8`-byte (`0x9` including the null terminator) ASCII string containing the dungeon located in it, padded with spaces on the right. For mixed pools : the string displays the hint area the dungeon is located in instead.|
